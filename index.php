...
<section class="tengah" id="tengah">
  <div class="container">
    ...
    <div class="row justify-content-center">
      ...
      <div class="row justify-content-center">
        ...
        $total_data = mysqli_num_rows(mysqli_query($koneksi,"SELECT * FROM tambah ORDER BY id DESC"));
        $total_laman = ceil($total_data/$per_laman);
        $awal = ($laman_sekarang - 1) * $per_laman; 


        $sql = mysqli_query($koneksi, "SELECT * FROM tambah ORDER BY id DESC limit $per_laman OFFSET $awal");
  ...
  <div class="card-footer"><p class="tanggal" style="float: left"><?php echo $data['tanggal']; ?></p></div>
</div>
    </div>
    <?php   
              }
  ...
  if (isset($total_laman)) {
   ...
   <?php if ($total_laman > 1) {
     ?>
      <center>
        <?php if ($laman_sekarang > 1) {
         ?>
        <a href="index.php?laman=<?php echo $laman_sekarang - 1 ?>" class="btn btn-dark" aria-label="Sebelumnya">
    ...
    <?php if ($laman_sekarang < $total_laman) {?>
      <a href="index.php?laman=<?php echo $laman_sekarang + 1 ?>" class="btn btn-dark" aria-label="Selanjutnya">
